...
describe('HostsTab jump column', () => {
  it('renders the proxyJump chain and a dash for hosts without one', async () => {
    const hosts = [
      makeHost('web-1', { proxyJump: ['bastion', 'gateway'] }),
      makeHost('db-1'),
    ]
    const api = { listHosts: vi.fn(async () => hosts) } as unknown as SshApi
    const container = document.createElement('div')
    document.body.appendChild(container)
    const root = createRoot(container)
    mountedRoots.push({ root })
